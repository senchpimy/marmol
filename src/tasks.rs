...
impl Default for TasksGui{
    fn default() -> Self {
        Self{
            ...
            new_task:String::new(),
            edit_task:EditDay{index:-1,edit: Edit::Null},
...
impl TasksGui{
    ...
    pub fn set_path(&mut self,path:&str) {
        if path!=&self.path{
            ...
            for element in &self.json_content.days {
                let mut tot:u16=0;
                element.tasks.iter().for_each(|val|{tot+=val.completed as u16});
                prom+=tot;
    ...
    pub fn show(&mut self, ui:&mut egui::Ui){
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ui.group(|ui|{
                ui.label(RichText::new("Tasks").color(ui.visuals().strong_text_color()).size(20.0));
                let mut del=0;
                    ...
                    ui.horizontal(|ui|{
                        ...
                        if ui.button("Add").clicked(){
                            ...
                            let desc:Option<String>=
                                if self.new_task_desc.is_empty(){
                                None
                            }else{
                                Some(self.new_task_desc.clone())
                            };
                            self.json_content.tasks.push(
                                TaskToDo{name:self.new_task.clone(),description:desc,id:new_id
                                });
                            self.tasks_hash.insert(new_id,self.new_task.clone());
                            if self.new_task_update{
                                for val in &mut self.json_content.days{
                                    val.tasks.push(TaskCompleted{completed:false,id:new_id});
                                }
                            }
                            self.new_task=String::new();
                ...
                ui.group(|ui|{
                    ...
                    ui.horizontal(|ui|{
                        ...
                        if ui.button("Add").clicked(){
                            ...
                            self.json_content.days.insert(0,Day::new(&self.new_entry_tittle,tasks));
                            self.add_entry=false;
                        }
                    });
                });
                ...
                ui.group(|ui|{
                        ui.horizontal_top(|ui|{
                            if self.edit_task.index == ind as i32 && self.edit_task.edit == Edit::Tittle{
                                if ui.add(
                                    egui::TextEdit::singleline(&mut self.edit)
                                    ).lost_focus(){
                                    self.edit_task=EditDay{index:-1,edit:Edit::Null};
                    ...
                    ui.add_space(10.0);
                    if self.edit_task.index == ind as i32 && self.edit_task.edit == Edit::Description{
                        ...
                        if ui.button("Edit desc").clicked(){
                            self.edit_task=EditDay{index:ind as i32,edit:Edit::Description};
