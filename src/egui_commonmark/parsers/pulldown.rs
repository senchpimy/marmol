...
use crate::egui_commonmark::{CommonMarkCache, CommonMarkOptions};

use egui::{self, Id, Pos2, TextStyle, Ui};

use crate::egui_commonmark::List;
use crate::egui_commonmark_backend::elements::*;
    ...
    fn blockquote<'e>(
        ...
        if self.is_blockquote {
            ...
            if let Some(alert) = parse_alerts(&options.alerts, &mut collected_events) {
                crate::egui_commonmark_backend::alert_ui(alert, ui, |ui| {
    ...
    fn start_tag(&mut self, ui: &mut Ui, tag: pulldown_cmark::Tag, options: &CommonMarkOptions) {
        match tag {
            ...
            pulldown_cmark::Tag::CodeBlock(c) => {
                match c {
                    pulldown_cmark::CodeBlockKind::Fenced(lang) => {
                        self.code_block = Some(crate::egui_commonmark::CodeBlock {
            ...
            pulldown_cmark::Tag::Link { dest_url, .. } => {
                self.link = Some(crate::egui_commonmark::Link {
