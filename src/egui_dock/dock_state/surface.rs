...
use crate::egui_dock::{Node, NodeIndex, Tree, WindowState};

/// A [`Surface`] is the highest level component in a [`DockState`](crate::egui_dock::DockState). [`Surface`]s represent an area
...
pub enum Surface<Tab> {
    ...
    /// The main surface of a [`DockState`](crate::egui_dock::DockState), only one should exist at surface index 0 at any one time.
    Main(Tree<Tab>),
