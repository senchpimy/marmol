...
use crate::egui_dock::dock_area::tab_removal::{ForcedRemoval, TabRemoval};
use crate::egui_dock::node::LeafNode;
use crate::egui_dock::{
    dock_area::{
...
use crate::egui_dock::tab_viewer::OnCloseResponse;
...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tabs(
        ...
        for tab_index in 0..tabs_len {
            ...
            let (is_active, label, icon, tab_style, closeable) = {
                let leaf = self.dock_state[surface_index][node_index]
                    .get_leaf_mut()
                    .expect("This node must be a leaf");
                ...
                (
                    ...
                    tab_viewer.icon(&mut leaf.tabs[tab_index.0]),
    ...
    fn tab_title(
        ...
        let icon_size = 18.0; 
        let icon_spacing = 5.0;
        let has_icon = icon.is_some();
        let content_width = galley.size().x + if has_icon { icon_size + icon_spacing } else { 0.0 };

        let text_width = content_width + 2.0 * x_spacing;
