...
fn toggle_ui(ui: &mut egui::Ui, con: &mut main_area::Content) -> egui::Response {
    ...
    if *con != main_area::Content::Graph {
        if ui.is_rect_visible(rect) {
            ...
            ui.painter()
                .rect(rect, radius, visuals.bg_fill, visuals.bg_stroke, kind);
...
fn toggle_ui_bool(ui: &mut egui::Ui, on: &mut bool) -> egui::Response {
    let desired_size = ui.spacing().interact_size.y * egui::vec2(1.5, 0.7);
    let (rect, mut response) = ui.allocate_exact_size(desired_size, egui::Sense::click());
    if response.clicked() {
        *on = !*on;
        response.mark_changed();
...
pub fn toggle_bool(on: &mut bool) -> impl egui::Widget + '_ {
    move |ui: &mut egui::Ui| toggle_ui_bool(ui, on)
}
